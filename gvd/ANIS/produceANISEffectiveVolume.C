...
int SaveHistograms(std::map<std::string,TH1D*> &histograms, std::map<std::string,TH2D*> &histograms2D, TString fileName)
{
    ...
    outputFileName = fileName(0,fileName.Last('/')) + "/effVolume_" + fileName(fileName.Last('/')+1,fileName.Length()) + ".root";
...
{
    int eventID,interactionType;
    float eventWeight=0,ENuZero=0,ENuIn=0,ECasc=0,V1x=0,V1y=0,V1z=0,p1x=0,p1y=0,p1z=0;
    string holder;
    ifstream readFile;
    ...
    while(readFile>>eventID){
        if (nEvents != -1 && eventID > nEvents)
            break;
        readFile>>eventWeight>>interactionType>>ENuZero>>ENuIn>>V1x>>V1y>>V1z>>p1x>>p1y>>p1z;
...
int produceANISEffectiveVolume(TString fileName = "/Data/BaikalData/mc/ANIS/nueatm_ver4_50kNoise/ANISoutput",int nEvents = -1)
{
